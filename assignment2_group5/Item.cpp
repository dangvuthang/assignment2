...
Item::Item(string id, string title, string rentalType, string loanType, int numberOfCopies, float rentalFee, string rentalStatus) {
	this->id = id;
	this->title = title;
...
int Item::GetNumberOfCopies() {
	return numberOfCopies;
}
float Item::GetRentalFee() {
	return rentalFee;
}
string Item::GetRentalStatus() {
	return rentalStatus;
}
void Item::SetId() {
	string input;
	do {
		...
		cin >> input;
		if (!Item::CheckId(input)) cout << "Invalid format. Format: Ixxx-yyyy (x, y are number). Try again" << endl;
...
void Item::SetRentalType(int type) {
	if (type == 1) this->rentalType = "Game";
	else if (type == 2) this->rentalType = "DVD";
	else if (type == 3) this->rentalType = "Record";
}
void Item::SetLoanType() {
	...
	do {
		...
		cin >> input;
		if (!(input == "1" || input == "2")) cout << "Invalid option. Try again" << endl;
