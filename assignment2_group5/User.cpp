...
vector<string> User::GetListOfRentals() {
...
void User::SetId() {
	...
	do {
		...
		cin >> input;
		if (!User::CheckId(input)) cout << "Invalid format. Format: Cxxx (x is a number). Try again" << endl;
...
void User::SetRole() {
	...
	do {
		...
		cin >> input;
		if (!(input == "1" || input == "2" || input == "3")) cout << "Invalid option. Try again" << endl;
...
void User::AddRental(string itemId) {
